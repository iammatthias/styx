---
name: skill-cleaner
description: Audits the styx skill and soul library — description budget, duplicates, orphans, name/dir drift. Returns a punch list. Use when Level-0 metadata feels heavy or before adding a skill.
---
...
5. **Compact the over-budget descriptions.** Relaxed grammar — drop articles, fold clauses, swap a full stop for a dash — until under cap. Keep every trigger noun. Keep the "use when" signal. Don't sand off the voice.

6. **Report. Don't auto-delete.** Hand back the punch list. Apply only what the user approves.
